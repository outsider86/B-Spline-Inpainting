...
def evaluate(
    ...
    if split not in {"train", "val", "test"}:
        raise ValueError("open-loop split must be 'train', 'val', or 'test'")
    ...
    report={
        ...
        "sampling_protocol":{"seed_reset_per_checkpoint":20260915,"batch_size":batch_size,"fm_steps":cfg.policy.fm_steps,
                             ...
                             "decoder":"deterministic MaskGIT confidence unmasking" if not is_continuous_architecture(cfg.policy.architecture) else "Euler flow integration"},
...
def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--config",default="configs/default.yaml"); p.add_argument("--set",action="append",default=[]); p.add_argument("--architecture",required=True,choices=TRAINABLE_ARCHITECTURES); p.add_argument("--checkpoint",required=True); p.add_argument("--max-samples",type=int); p.add_argument("--batch-size",type=int,default=64); p.add_argument("--split",choices=("train","val","test"),default="test"); p.add_argument("--output",required=True)
