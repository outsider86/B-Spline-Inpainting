#!/usr/bin/env python3
"""Prepare and train V5/V5.1 state+previous-command FM checkpoints.

V5 uses two observation timesteps (four RGB images). V5.1 deliberately uses
only the current timestep (two RGB images). Both protocols share immutable
action/RGB caches because their episode splits, targets, and source frames are
identical; checkpoints, logs, and W&B runtime files remain version-isolated.
"""
...
class DatasetSpec:
    ...
    config_family: str = "bsp_unet_v5"

    @property
    def config_root(self) -> Path:
        return ROOT / "configs" / self.config_family / self.config_name
...
def _experiment(protocol: str) -> tuple[tuple[DatasetSpec, ...], tuple[str, ...], Path, int]:
    if protocol == "v5":
        return BASE_SPECS, ("fm_raw_h2", "fm_bspline_h2"), ROOT / "outputs" / "BSP_UNET_V5_STATUS.json", 2
    if protocol != "v5.1":
    ...
    specs = tuple(
        DatasetSpec(
            ...
            output_name=spec.output_name.replace("BSP_UNET_V5_", "BSP_UNET_V5_1_"),
            ...
            config_family="bsp_unet_v5_1",
        )
        for spec in BASE_SPECS
    )
    return specs, ("fm_raw_h1", "fm_bspline_h1"), ROOT / "outputs" / "BSP_UNET_V5_1_STATUS.json", 1
...
def _prepare_actions(spec: DatasetSpec, variant: str) -> None:
    ...
    prepared = Path(load_config(config).data.prepared_path)
...
def _prepare_rgb(spec: DatasetSpec) -> None:
    variant = "fm_raw_h1" if spec.config_family == "bsp_unet_v5_1" else "fm_raw_h2"
    config = spec.config_root / f"{variant}.yaml"
    output = Path(load_config(config).data.rgb_cache_path)
...
def prepare_all(specs: tuple[DatasetSpec, ...], variants: tuple[str, ...]) -> None:
    failures: list[str] = []
    lock = threading.Lock()
...
def _verify_models(
    spec: DatasetSpec, variant: str, checkpoint: Path, observation_horizon: int
) -> None:
    final = torch.load(checkpoint, map_location="cpu", weights_only=False, mmap=True)
    try:
        already_finalized = final.get("checkpoint_kind") == "best_validation_model"
        if final.get("config", {}).get("data", {}).get("state_dim") != 14:
            raise ValueError(f"{checkpoint} is not a 14D V5 checkpoint")
        if final.get("config", {}).get("data", {}).get("observation_horizon") != observation_horizon:
            raise ValueError(f"{checkpoint} has the wrong observation horizon")
    ...
    for epoch, path in zip(BEST_EPOCHS, expected):
        ...
        try:
            ...
            if payload.get("config", {}).get("data", {}).get("observation_horizon") != observation_horizon:
                raise ValueError(f"{path} has the wrong observation horizon")
...
def _finalize_best_model(checkpoint: Path) -> None:
    """Publish only the lightweight global validation winner as ``base.pt``."""

    best = checkpoint.with_suffix(checkpoint.suffix + ".best.weights.pt")
    payload = torch.load(checkpoint, map_location="cpu", weights_only=False, mmap=True)
    try:
        already_finalized = payload.get("checkpoint_kind") == "best_validation_model"
    finally:
        del payload
    if not already_finalized:
        if not best.is_file():
            raise FileNotFoundError(f"missing final validation-best weights: {best}")
        ...
        try:
            if selected.get("checkpoint_kind") != "best_validation_model":
                raise ValueError(f"{best} is not a standalone validation-best model")
        finally:
            del selected
    ...
    # Training writes its manifest before this lightweight promotion. Refresh
    # the file hash so the deployment audit remains authoritative.
...
def _task_instruction(spec: DatasetSpec) -> str:
    return {
...
def finalize_existing(
    ...
    for spec in specs:
        for variant in variants:
            ...
            _verify_models(spec, variant, checkpoint, observation_horizon)
            _finalize_best_model(checkpoint)
            write_server_config(
                checkpoint,
                checkpoint.with_suffix(".server.json"),
                task_instruction=_task_instruction(spec),
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--finalize-existing",
        action="store_true",
        help="promote completed validation-best models and remove training state",
    )
    ...
    specs, variants, status_path, observation_horizon = _experiment(args.protocol)

    if args.finalize_existing:
        finalize_existing(specs, variants, observation_horizon)
        return

    if not args.skip_prepare:
        prepare_all(specs, variants)
    ...
    status = {
        ...
        "protocol": {
            ...
            "images_per_example": 2 * observation_horizon,
    ...
    def gpu_worker(gpu: int) -> None:
        while True:
            ...
            try:
                _train_worker(
                    spec, variant, gpu, status, lock, status_path, observation_horizon
                )
            finally:
                pending.task_done()
    ...
    _atomic_json(
        ROOT / "outputs" / (
            "BSP_UNET_V5_COMPLETE.json"
            if args.protocol == "v5"
            else "BSP_UNET_V5_1_COMPLETE.json"
        ),
        {
            "completed_unix": time.time(),
